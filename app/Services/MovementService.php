...
class MovementService
{
    ...
    public static function getAllMouvementMonthly($console, $date_start_month, $date_end_month, $logger, $id_planning)
    {
        $geoloc_service = new GeolocalisationService();
        $utils = new Utils();
        ...
        try {
            Vehicule::where('id_planning', $id_planning)
            ->chunk(50, function ($all_trucks) use ($console, $geoloc_service, $utils, $date_start_month, $date_end_month, $logger, &$insertData) {
                $console->withProgressBar($all_trucks, function ($truck) use ($geoloc_service, $console, $utils, $date_start_month, $date_end_month, $logger, &$insertData) {
                    try {
                        $alreadyProcessed = DB::table('movement')
                            ->where('imei', $truck->imei)
                            ->whereBetween('start_date', [$date_start_month, $date_end_month])
                            ->exists();

                        if ($alreadyProcessed) {
                            $logger->info("Camion {$truck->imei} déjà traité, on passe.");
                            return; // passe au camion suivant
                        }
                        ...
                        if (!$drive_and_stops || (empty($drive_and_stops['drives']) && empty($drive_and_stops['stops']))) {
                            ...
                            $logger->warning('Aucun mouvement trouvé pour le camion', [
                                ...
                                'nom' => $truck->nom,
                                'periode' => [
                                    'start' => $date_start_month->format('Y-m-d'),
                                    'end' => $date_end_month->format('Y-m-d'),
                                ],
                            ]);
                            return;
                        }
            ...
            $logger->critical('Erreur globale', ['message' => $e->getMessage()]);
            throw $e;
        }
    }
    ...
    public static function getMissingMouvementMonthly($console, $date_start_month, $date_end_month, $id_planning)
    {
        $geoloc_service = new GeolocalisationService();
        $utils = new Utils();
        $insertData = [];
        $month_start = (int) $date_start_month->format('m');
        $month_end = (int) $date_end_month->format('m');

        $all_trucks = Vehicule::where('id_planning', $id_planning)
        ->whereNotExists(function ($query) use ($month_start, $month_end) {
            $query->select(DB::raw(1))
                  ->from('movement')
                  ->whereRaw('movement.imei = vehicule.imei')
                  ->whereMonth('movement.start_date', $month_start)
                  ->whereMonth('movement.end_date', $month_end);
        })->get();
