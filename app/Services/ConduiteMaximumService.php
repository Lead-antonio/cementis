...
class ConduiteMaximumService
{
     ...
     *
     */
    public function CheckDriveMaxDayAndNight() {
        ...
        $nightStart = Carbon::createFromTime(22, 0, 0); // 19h00
        ...
        if ($dayDrivingDuration > 13 * 3600) { // 13 heures en secondes
            $this->registerInfraction($movement->rfid, $movement->imei, 'Conduite maximum dans une journée de travail', $dayDrivingDuration, $date, $movements, 13 * 3600);
        }
    
        // Vérifier les infractions pour conduite de nuit
        if ($nightDrivingDuration > 12 * 3600) { // 12 heures en secondes
            $this->registerInfraction($movement->rfid, $movement->imei, 'Conduite maximum dans une journée de travail', $nightDrivingDuration, $date, $movements, 12 * 3600);
        }
    }
    ...
    private function registerInfraction($driverId, $vehicleId, $eventType, $duration, $date, $movements, $maxDuration) {
        ...
        $points = (int)($excessDuration / $penalite->param); // Convertir en points
        ...
        foreach ($movements as $movement) {
            Infraction::create([
                ...
                'distance_calendar' => 0,
                'odometer' => 0,
                'duree_infraction' => gmdate('H:i:s', $excessDuration), // Durée d'infraction = excès
                ...
                'duree_initial' => $movement->duration,
                'insuffisance' => 0, // Ajuster si nécessaire
                'commentaire' => 'Infraction détectée pour durée de conduite excessive',
            ]);
        }
